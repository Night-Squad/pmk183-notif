...
@RestController
@RequestMapping("/api/cetak-manual/setoran-awal")
public class CetakManualSetoranAwalController extends HibernateReportController<SetoranAwal, SetoranAwalDTO> {
    ...
    public CetakManualSetoranAwalController(StorageService storageService) {
    ...
    public Map<String, Object> getData(HibernateDataUtility dataUtility, HibernateDataSource<SetoranAwal, SetoranAwalDTO> dataSource) throws Exception {
        ...
        Date tglTransaksi = new SimpleDateFormat("dd/MM/yyyy").parse(dataSource.getRequestParameterValue("tglTransaksi"));
        ...
        cetakSetoranAwalHajiData.setNoRekening("0");
        TipeHaji tipeHaji = tipeHajiRepository.findById(tipeHajiId).orElse(null);
        cetakSetoranAwalHajiData.setJenisHaji(tipeHaji.getKodeHaji());
        cetakSetoranAwalHajiData.setSettlementDate(new Date());
        Channel channel = channelRepository.findById(1L).orElse(null);
        cetakSetoranAwalHajiData.setMerchantType(channel.getKodeMerchant());
        cetakSetoranAwalHajiData.setTerminalId(userBranchCode);
        cetakSetoranAwalHajiData.setTanggalTransaksi(new SimpleDateFormat("ddMMyyyy").format(tglTransaksi));
        cetakSetoranAwalHajiData.setNomorValidasi(noValidasi);
        ...
        innerResult.put("no_validasi", noValidasi);
        innerResult.put("bank", data.getNamaBank().trim().toUpperCase());
        innerResult.put("no_rekening", data.getNoRekening());
