...
public interface MasterTxRepository extends JpaRepository<MasterTx, Integer> {
    ...
    @Query(value = "SELECT * FROM master_tx WHERE archive_no = :tx_reference_no AND (is_reversal <> true OR is_reversal IS NULL) and created_at BETWEEN :start_date AND :end_date ORDER BY created_at DESC, id DESC", nativeQuery = true)
